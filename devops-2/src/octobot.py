...
bot = telebot.TeleBot("6699616369:AAF3WwwmTxb96hg2tOYwaKgJRLA_HAiml6Q")
...
def connect_table(message):
    ...
    url = message.text
    if is_valid_url(url) and "docs.google.com/spreadsheets" in url:
        ...
        bot.send_message(message.chat.id, "Введена некорректная url")
    start(message)
...
def choose_action(message):
    ...
    if message.text == "Подключить Google-таблицу":
        info = bot.send_message(message.chat.id, "Введите ссылку на google-таблицу")
        ...
        info = bot.send_message(message.chat.id, "Что вы собираетесь делать с предметом?", reply_markup=start_markup)
        ...
        info = bot.send_message(message.chat.id, "Что вы собираетесь делать с дедлайном?", reply_markup=deadline_markup)
...
def choose_subject_action(message):
    ...
    if message.text == "Удалить предмет":
        if df.empty:
            bot.send_message(message.chat.id, "Таблица пуста, удалять нечего")
            start(message)
        else:
        ...
        start_markup = telebot.types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
        if df.empty:
            bot.send_message(message.chat.id, "Таблица пуста, изменять нечего")
            start(message)
        else:
...
def show_deadlines(message):
    ...
    text_table = []
    if df.empty:
        bot.send_message(message.chat.id, "Таблица пуста, дедлайнов нет")
    else:
            ...
            for i in range(1, d.shape[1] + 1):
                if d[str(i)][index]:
                    ...
                    if date >= today and (date - today).days <= 7:
                        dates_text += f"          {i} работа >> {d[str(i)][index]} просрочится на этой неделе\n"
                    else:
...
def choose_deadline_action(message):
    ...
    df = access_current_sheet()[2]
    if df.empty and message.text != "Главное меню":
        bot.send_message(message.chat.id, "Нет предметов, к которым можно добавить дедлайн. Добавьте предметы!!")
        start(message)
    else:
        if message.text == "Удалить дедлайн":
            info = bot.send_message(
                message.chat.id, "Введите название предмета и номер работы через ,"
            )
...
def choose_removal_option(message):
    ...
    warning = bot.send_message(message.chat.id, "Вы уверены, что хотите удалить ВСЮ таблицу? да/нет")
...
def choose_subject(message):
    ...
    if not df["Subject"].isin([old_sub]).any():
        ...
        bot.send_message(message.chat.id, "Такого предмета у нас нет")
        return start(message)
    if old_sub == "Главное меню":
        return start(message)

    info = bot.send_message(message.chat.id, "Напишите номер работы и новый дедлайн через , dd/mm/yy")
...
def update_subject_deadline(msg, old_sub):
    ...
    try:
        ...
        if not 1 <= int(n) <= 5:
            bot.send_message(msg.chat.id, "Номер работы должен быть от 1 до 5")
            return start(msg)
...
def delete_deadline(msg):
    ...
    try:
        ...
        if not df["Subject"].isin([sub]).any():
            bot.send_message(msg.chat.id, "Такого предмета у нас нет")
            return start(msg)
        ...
        bot.send_message(
            msg.chat.id, "Введите через , номер работы от 1 до 5"
        )
        start(msg)
...
def add_new_subject(message):
    ...
    elif df["Subject"].isin([message.text]).any():
        bot.send_message(message.chat.id, "Такой предмет у нас уже есть")
        return start(message)
    else:
        worksheet.append_row([message.text])
        info = bot.send_message(message.chat.id, "Добавьте url предмета")
...
def add_new_subject_url(message):
    ...
    if is_valid_url(message.text):
        ...
        bot.send_message(message.chat.id, "Новая ссылка внесена")
        start(message)

    else:
        bot.send_message(message.chat.id, "Ссылка некорректная")
        start(message)
    ...
    def new_subject_and_url(msg):
        try:
            ...
            if is_valid_url(url):
                ...
                bot.send_message(msg.chat.id, "Ссылка некорректная")
                start(msg)
        except ValueError:
            bot.send_message(msg.chat.id, "Введите через ,")
            start(msg)

    info = bot.send_message(message.chat.id, "Введите через , название предмета и ссылку")
...
def clear_subject_list(message):
    ...
    if message.text.lower() == "да":
        ...
        bot.send_message(message.chat.id, "Хорошо, чистить не будем")
        start(message)

    else:
        bot.send_message(message.chat.id, "Вводить нужно да или нет")
